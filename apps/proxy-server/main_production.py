...
SESSION_SECRET_KEY = os.getenv("SESSION_SECRET_KEY", "test-session-secret-for-development")  # For n8n JWT validation
JWT_ALGORITHM = "HS256"
...
async def forward_to_n8n_stream(message: str, jwt_token: str, session_data: dict):
    ...
    try:
        ...
        jwt_payload = jwt.decode(jwt_token, SESSION_SECRET_KEY, algorithms=[JWT_ALGORITHM])
        ...
        async with httpx_client.stream(
            ...
            if response.status_code == 200:
                ...
                async for chunk in response.aiter_bytes(chunk_size=1):  # Byte-by-byte for immediate processing
                        ...
                        if line:
                            try:
                                ...
                                if chunk_type == "begin":
                                    ...
                                    logger.info(f"Streaming started for node: {json_obj.get('metadata', {}).get('nodeName')}")
                                    
                                elif chunk_type == "item":
                                    # Stream content immediately as it arrives from n8n
                                    content = json_obj.get("content", "")
                                    if content:
                                        logger.debug(f"Chunk from n8n: {repr(content[:20])}")
                                    ...
                                    logger.info(f"Streaming ended for node: {json_obj.get('metadata', {}).get('nodeName')}")
