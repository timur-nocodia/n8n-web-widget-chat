...
def create_n8n_token(session_id: str, message_history: list = None, session_metadata: dict = None) -> str:
...
async def create_session(
    ...
    session_id = session_data.session_id or f"sess_{int(time.time())}_{str(uuid.uuid4())[:8]}"
...
async def stream_chat(
    ...
    n8n_token = create_n8n_token(
        session_id, 
        message_data.message_history,
        message_data.session_metadata
    )
